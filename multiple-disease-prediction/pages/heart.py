...
heart_csv_path = os.path.abspath(os.path.join(working_dir, "..", "dataset", "heart.csv"))

try:
    heart_data = pd.read_csv(heart_csv_path)
except FileNotFoundError:
    st.error(f"Error: File not found at {heart_csv_path}. Please check the file location.")
    st.stop()
...
heart_model_path = os.path.join(working_dir, "heart_disease_model.sav")
pickle.dump(model, open(heart_model_path, 'wb'))

# Streamlit UI for Heart Disease Prediction
st.title("Heart Disease Prediction")

# User inputs with Initial Values as Zero
...
trestbps = st.number_input("Resting Blood Pressure", min_value=80, max_value=200, value=80)
chol = st.number_input("Serum Cholesterol in mg/dl", min_value=100, max_value=600, value=100)
fbs = st.selectbox("Fasting Blood Sugar > 120 mg/dl", [0, 1])
restecg = st.number_input("Resting Electrocardiographic results", min_value=0, max_value=2, value=0)
thalach = st.number_input("Maximum Heart Rate achieved", min_value=60, max_value=220, value=60)
...
thal = st.number_input("Thal: 0 = normal; 1 = fixed defect; 2 = reversible defect", min_value=0, max_value=3, value=0)
